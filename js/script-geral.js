...
function initWaterCalculator() {
  ...
  if (!form) return;
  const inputs = { consumo: form.querySelector('#agua-consumo'), economia: form.querySelector('#agua-reutilizada'), tarifa: form.querySelector('#agua-tarifa') };
  const results = { custo: form.querySelector('.results-box .result-item:nth-of-type(1) .negrito'), economia: form.querySelector('.results-box .result-item:nth-of-type(2) .negrito') , pegada: form.querySelector('.results-box .result-item:nth-of-type(3) .negrito') };
  function calculate() {
    ...
    results.custo.textContent = `R$ ${custoEstimado.toFixed(2).replace('.', ',')}`;
    results.economia.textContent = `R$ ${Math.abs(economiaEstimada).toFixed(2).replace('.', ',')}`;
...
function initDashboards() {
  ...
  charts.water = new Chart(waterCtx, {
    type: 'bar',
    data: { labels: labels, datasets: [{ label: 'Projeção de Economia', data: [], backgroundColor: '#cc2e2e', stack: 'stack1' }, { label: 'Gasto Líquido Estimado', data: [], backgroundColor: '#2ecc71', stack: 'stack1' }] },
    options: { responsive: true, maintainAspectRatio: false }
  });
